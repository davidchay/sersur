...
if ( ! function_exists( 'understrap_bootstrap_comment_form_fields' ) ) {
	...
	function understrap_bootstrap_comment_form_fields( $fields ) {
		...
		$replace = array(
			'<p class="' => '<div class="my-5 flex gap-4 items-center ',
			'<label ' => '<label class=" w-1/4 text-right"',
			'<input'     => '<input class="rounded py-2 px-3 w-full bg-white border border-gray-300" ',
			'</p>'       => '</div>',
		);
		...
		$replace = array(
			...
			'<input'     => '<input class="bg-white border border-gray-300" ',
...
if ( ! function_exists( 'understrap_bootstrap_comment_form' ) ) {
	...
	function understrap_bootstrap_comment_form( $args ) {
		$replace = array(
			'<p class="' => '<div class="my-5 flex gap-4 items-start ',
			'<label ' => '<label class=" w-1/4 text-right " ',
			'<textarea'  => '<textarea class="rounded py-2 px-3 w-full h-16 bg-white border border-gray-300" ',
			'</p>'       => '</div>',
		);
		...
		if ( isset( $args['class_submit'] ) ) {
			$args['class_submit'] = 'bg-secondary py-3 my-4 hover:cursor-pointer hover:bg-secondary-dark px-3 text-white rounded-lg';
		}
