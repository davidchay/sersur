	...
	function understrap_pagination( $args = array(), $class = 'pagination' ) {
		...
		$args = wp_parse_args(
			...
			array(
				'mid_size'           => 2,
				'prev_next'          => true,
				'prev_text'          => __( '&larr;', 'understrap' ),
				'next_text'          => __( '&rarr;', 'understrap' ),
				'type'               => 'array',
		...
		<nav aria-labelledby="posts-nav-label" >
			...
			<ul class="<?php echo esc_attr( $class ); ?> my-5 flex justify-center items-center">

				<?php
				foreach ( $links as $key => $link ) {
					
					?>
					<?php if( strpos( $link, 'dots')): ?>
						<li class="page-item  ">
					<?php else: ?>
						<li class="page-item text-white rounded-lg hover:bg-secondary-dark mx-2 <?php echo strpos( $link, 'current' ) ? 'active bg-secondary-dark' : 'bg-secondary'; ?>">
					<?php endif; ?>
						<?php echo str_replace( 'page-numbers', 'page-link block py-2 px-4 text-lg', $link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</li>
					<?php
				}
				?>
